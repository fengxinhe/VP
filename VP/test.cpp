...
Mat computeVpScore(string filePath)
{
	...
	// Scale(down sampling)
	printf("img rows=%d, cols=%d ", img_gray.rows, img_gray.cols);
	Mat image(img_gray.rows * scale_factor, width, CV_32F);
	resize(img_float, image, image.size());*/
	//int width = 128;
	//int height = 85;
	float scale_factor_width = (float)width / (float)image_origin.cols;
	float scale_factor_height = (float)height / (float)image_origin.rows;
	printf("img rows=%d, cols=%d ", img_gray.rows, img_gray.cols);
	...
	cv::circle(image_origin, cvPoint(p_max.x / scale_factor_width, p_max.y / scale_factor_width), 10, Scalar(0), 5, 8, 0);
	//cv::circle(img_resize, cvPoint(p_max.x, p_max.y), 10, Scalar(0), 5, 8, 0);
