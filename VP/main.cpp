...
Mat computeVpScore(Mat image_origin, Mat image)
{
    ...
    if (p1.x==-1){
        ...
        p1.y= p_max.y / scale_factor_width;
    }
    p2.x=p_max.x / scale_factor_width;
    p2.y= p_max.y / scale_factor_width;
    points[pnum].x=p_max.x / scale_factor_width;
    points[pnum].y= p_max.y / scale_factor_width-30;
    pnum++;
...
Mat drawPointsAndLine(Mat img){

    for( int i=0;i<pnum;i++){
        cv::circle(img, points[i], 10, Scalar(255,255,255), 5, 8, 0);
    }
    cv::line(img, cvPoint(p1.x-300,p1.y),cvPoint(p2.x+300,p2.y), Scalar(0),3,8,0);
...
int segmentRoad(Mat image_origin, Mat image) {
    ...
    image_origin=drawPointsAndLine(image_origin);
